...
import ProductsCardRow from "../productsCardRow";
...
import ProductsCardColum from "../productsCardColum";
...
const NewProducts = () => {
  return (
    <section className={styles.newProductsAllDataContainer}>
      <div>
        <h1>New Products</h1>
      </div>

      <div className={styles.newProductsCartContainer}>
        {newProducts.slice(0, 6).map((product, ind) => (
          <ProductsCardRow key={ind} productCardData={product} />
        ))}
      </div>
    </section>
  );
